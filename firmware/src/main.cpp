...
ISR(TIMER1_COMPA_vect) {
  ...
  if (heater_timer_acc >= 333333L) { //здесь 475к вместо 500 потому что прибавляем не по 1 а примерно по 25к
    heater_timer_acc = 0;
    // Начало нового цикла 3 Гц
...
// 10  раз в секунду
  ...
  // Обновляем экран только когда есть новые данные длины протянутого филамента
  if (newDataFlag) {
